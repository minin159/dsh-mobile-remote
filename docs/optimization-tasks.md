...
2. D:\dsh-plugins\mobile-remote-app\docs\ui-mockup-v3-gray.png（视觉基准——壳已按此实现）
3. docs/phase-4-spec.md / phase-5-spec.md（页面现有结构：首页工作区卡片、会话视图、底栏）

设计令牌（与壳一致）：背景 #000 / 卡片 #1C1C1E / #2C2C2E · 正文 #F2F2F7 · 次级灰 #8E8E93 ·
分割线 #38383A · 主强调灰 #8E8E93 · 开关绿 #30D158 · 错误红 #FF453A · 字体栈
...
- 会话视图：气泡（用户 #0A84FF 系灰调可换 #48484A、AI #1C1C1E）、代码块/表格圆角卡片化、
...
冒烟必须全过（72 断言），任一失败先修样式引用再继续；同一问题重试 ≤2 次；超线即停汇报。
...
**执行顺序建议**：大验收收官 → 大任务 A → 大任务 B → 大任务 C（内容层最后做，
此时 App 壳 v3 与页面一起构成完整 Apple 体验）。C 与 A/B 同仓库同文件，仍然串行。
