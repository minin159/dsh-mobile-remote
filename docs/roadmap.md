...
| C1 真机反馈修复轮 ✅ | 四条真机反馈（基于 v1.6.0）：新建会话默认 D 盘 / 思考可折叠块 / 进会话回读历史 / APK 滚动失效；优4 本轮用户叫停不执行（WIP 留 opt4-wip） | 目标 25–40 万 | 实现+自检完成（2026-09-03，tag `c1-fixes-done`，v1.7.0，四条各独立 commit）：① /new 默认 `NEW_SESSION_CWD='D:\\'` 常量（显式 cwd 以请求为准，不做设置页配置）；② 思考块流式累积 + 定稿收起摘要 + localStorage 折叠记忆（载荷形状宿主源码核实：定稿 content 含 reasoning 块）；③ readSession 载荷核实（`{session,events[]}` live 优先）→ 缓冲 <30 帧拉最近 30 条定稿消息以独立 history 帧推送，页面顶部「历史」分隔条渲染，决策 12 游标语义不变，读失败静默降级；④ APK 滚动根因 = 壳沉浸式全屏下 WebView 视口=整块物理屏、height:100% 超可视区 → 溢出不发生（诊断 `docs/fix4-scroll-diagnosis.md`），页面侧三层加固（100dvh + body overflow:hidden + fixed 兜底），壳侧 patch（adjustResize 等）按跨仓库约束交回主会话。冒烟 133 断言全绿（phase2 26 + phase4 69 + phase5 19 + opt2 19）。真机验收清单待回填（CHANGELOG 1.7.0 节）：D 盘落位 / 思考块展开定稿保留 / 历史区 + 无缝衔接 / APK 滑到底 / 浏览器无回归 |
| C2 真机反馈修复轮 II ✅ | 两条真机反馈（基于 v1.7.0）：新建会话 EPERM（盘符根 mkdir 被 Windows 拒绝）/ 模型切换改居中小弹窗可滑动 | 目标 6–12 万（实际约 4–5 万） | 实现+自检完成（2026-09-04，tag `c2-fixes-done`，v1.7.1，两条各独立 commit）：① fix(new-session) `21177f4`——NEW_SESSION_CWD 由 'D:\' 改 'D:\dsh-sessions'（固定子目录），enabled 变 true 三路径（设置热切换 / 启动预建 / /new 兜底）ensureNewSessionDir mkdir recursive 一次（失败记日志不阻塞），/new 502 报错带目标目录路径便于真机定位；② feat(model-picker) `e6f192a`——#modelSheet 全宽贴底 sheet → 居中模态小窗（宽 min(320px,86vw)、圆角 14px、max-height 76vh、半透明毛玻璃遮罩点击关闭），#modelList 独立滚动区（40vh + overflow-y auto + touch 惯性 + overscroll contain），条目 44px、当前项描边+对勾、点选即关+toast 确认；切换协议（POST 官方 selectModel，P8 证实路径）零改动；风格 E 令牌沿用不引入新色；shieldSheet 形态不动。冒烟 136 断言全绿（phase2 26 + phase4 70 + phase5 19 + opt2 21；本轮新增 E7/O2.1a2/O2.1a3）。真机验收清单待回填（CHANGELOG 1.7.1 节）：/new 成功且落 D:\dsh-sessions / 弹窗居中可滑 / 点选即切下一回合生效 |
| C3 五条新需求 ✅ | 视觉对齐 ZCode 移动端（字号收紧 + 底栏扁平化）+ 思考过程自动堆叠 + 空会话不存档 + 移动端删除会话（基于 v1.7.1 + 供应商堆叠） | 目标 15–25 万 | 实现+自检完成（2026-09-05，tag `c3-features-done`，v1.8.0，五条各独立 commit）：① style `11feb39` 字号——正文 15→14px 各层级等比收紧、列表行更密（纯 CSS）；② style `8e283b0` 底栏——去描边纯毛玻璃、图标键 transparent 扁平、mask 图标 22→18px/stroke 1.25、键 44→36px（纯 CSS）；③ feat(thinking) `3903f1c` 堆叠——容器粒度改每 turn 一个（thinkStacks），收起合并摘要条「思考 · N 段 · M 字」、定稿段瘦身只留摘要、重放幂等（chars 记账）；④ feat(new-session) `3ab6451` 空会话不存档——/new 记账 newCreatedSids（有界 32）+ POST /drop 双端校验（页面端零消息判定 + 服务端记账/无 user/message 复核）→ sessionController.drop，失败静默；⑤ feat(delete) `20abd8c` 删除——任务行长按 → 居中确认弹窗（同模型弹窗形态）→ POST /delete 三道服务端守卫（live 409/非 persisted 409/绑定 409）→ drop，live 态弹窗内置灰；审计 session_delete（含 sid 不含内容）。新增 probe/smoke-c3.mjs 26 断言（空会话 drop/删除守卫/页面端 vm 交互三组）。冒烟 162 断言两轮全绿（旧 136 + 新 26）。真机验收清单待回填（CHANGELOG 1.8.0 节）：视觉气质/思考堆叠/空会话不出现/长按删除 + 二次确认/运行中禁删 |
...
21. **优4 多设备观察者模式暂缓（优化B/优4，2026-09-03 用户叫停）**：已实现部分（新连接默认观察者、1 主控 + ≤3 观察者有界、接管 + 30s 冷却、clid 同页恢复护住 P5 息屏重载主路径、connKey 控制接口守卫、审批只推主控护住决策 11、observer_join/leave/takeover 审计）保留在 `opt4-wip` 分支（smoke-opt4 29 项过；smoke-phase5 回归段未按新语义改完、真机未验）；恢复实施时从该分支续做，不重推导。**C1 期间（2026-09-03）用户再次叫停，优4 不随 v1.7.0 发布。**
22. **历史回读帧独立于游标体系（C1，feat(history)）**：`history` SSE 帧（readSession 最近 30 条定稿消息）不带 seq、不进环形缓冲、不参与 cursorBySid——是只读的历史区渲染通道；live 流与决策 12 的补发语义完全不变。触发条件 = 环形缓冲 < 30 帧（缓冲已覆盖近期窗口就不重复拉）；页面按 sessionId 匹配 + histSep 去重 + 迟到帧丢弃。readSession 载荷形状已核实（`{session: header, events: [{type,seq,time,data}]}`，live 优先），不必再探。
23. **思考块载荷事实（C1，feat(thinking)）**：宿主定稿 assistant/message 的 content 数组保留 `{type:'reasoning', text}` 块（BlockAssembler 不丢弃，pi-ai thinking_* 全链映射），`assistant/chunk` 的 reasoning-delta 形状为 `{type:'reasoning-delta', index, text}`（text 为增量）。页面思考块定稿以定稿 reasoning 为准、无则用流式累积；模型不输出思考时不渲染任何块。
24. **APK 滚动失效根因与页面侧基准（C1，fix(scroll)，诊断 `docs/fix4-scroll-diagnosis.md`）**：壳沉浸式全屏（SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN）下 Android WebView 视口 = 整块物理屏，浏览器 App 则自行扣除系统栏——`html/body height:100%` 在 WebView 里超可视区，#stream flex:1 分到超视口高度 → 溢出不发生 → overflow-y:auto 不触发。页面基准自此为 `@supports 100dvh`（老内核回退 100%）+ `body overflow:hidden`（单滚动链）+ dvh 实效探测失败的 `position:fixed` 兜底；壳侧 patch（windowSoftInputMode adjustResize 等）按跨仓库约束交回主会话执行。后续任何页面布局改动不得回退该高度基准。
25. **思考堆叠粒度 = 每回合一个容器（C3，feat(thinking)）**：thinkStacks 按 turn 归并（不再每 step 一块）；收起态合并摘要条「思考 · N 段 · M 字」、展开段落纵向排列；定稿段瘦身为「第 N 段 · X 行 · Y 字」摘要行（全文 DOM 弃置，流式期间照旧累积）；重放幂等靠 seg.chars 记账（chunk 重演/重复定稿零漂移），定稿段的重放增量只入账不回显。localStorage 折叠偏好沿用 C1。
26. **会话删除/丢弃 = sessionController.drop 双门禁（C3，feat(delete)+feat(new-session)）**：drop 是删会话与丢空会话共用的唯一 API（任务给定，dsh-api-session-controller/lib/client.js:1538，live 注册表删除 + drainSessionDisposals）。/drop（空会话不存档）双端校验——页面端只对「本页 /new 创建且零消息」发（sendBeacon 优先），服务端复核 newCreatedSids 记账（有界 32）+ 环形缓冲无 user/message；**电脑端建的会话绝不在记账内**（409 拒绝）。/delete（显式删除）只放行「persisted 且非当前绑定」——live 运行中 409（页面端弹窗删除键置灰 + 服务端兜底）；删除审计 session_delete 只记 sid 不记内容。两路径 drop 失败均静默/如实报错，不阻塞。
27. **移动端删除交互 = 长按而非滑动（C3，feat(delete)）**：iOS 滑动删除在 Android WebView 手势滚动冲突下 touch 事件表现不稳定，选 contextmenu 长按（实现简单可靠）；确认弹窗复用模型弹窗形态（居中小窗 + 遮罩点击关闭 + 44px 按键）。删除后列表即时移除 = sessionsById 本地剔除 + renderHome 重建 + fetchSessions 校准（失败路径也刷新——多为 live 态变化的过期数据）。
